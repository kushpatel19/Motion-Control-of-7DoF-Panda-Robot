...
%     ddQdt(j,:) = -sin(t);
% end
% Straight line task space trajectory parameters
x0 = -0.009454490681018238; 
y0 = 0.04346090637134166;
zf = 0.0633260626183437; % Fixed z-coordinate
xf = -0.1;
yf = -0.1;

% Coefficient Matrix [Cubic trajectory]
A = [1, 0,  0,    0;
     0, 1,  0,    0;
     1, t_fin, t_fin^2, t_fin^3;
     0, 0,  2*t_fin, 3*t_fin^2];
bx = [x0; 0; xf; 0];
by = [y0; 0; yf; 0];
bz = [0; 0; zf; 0];
tcx = (A)\bx;
...
for i=1:length(t)    
    ...
    X_desired(:, i) = [[1,t(i),t(i)^2,t(i)^3]*tcx; [1,t(i),t(i)^2,t(i)^3]*tcy ; zf; 0; 0; 0];
    X_dot_desired(:, i) = [[0,1,2*t(i),3*t(i)^2]*tcx; [0,1,2*t(i),3*t(i)^2]*tcy; 0; 0; 0; 0];
    X_ddot_desired(:, i) = [[0,0,2,6*t(i)]*tcx; [0,0,2,6*t(i)]*tcy; 0; 0; 0; 0];
    q(:,1) = [-2.8973; -1.7628; -2.8973; -3.0718; -2.8973; -0.0175; -2.8973];
    q_actual(:,1) = q(:,1);
    Jd = computeNumericalJacobian(q(:,i));
    dq(:,i) = pinv(Jd)*(X_dot_desired(:,i));
    q(:,i+1) = IK_Panda(q(:,i), dq(:,i));
    dq_actual(:,1) = dq(:,1);
    [xd,yd,zd] = FK_Panda(q(:,i));
    Computed_Traj(:,i) = [xd,yd,zd];
end

for i=1:length(t)
    
    if i<last
        ddq(:,i) = (dq(:,i+1)-dq(:,i))/delta_t;
    else
        ddq(:,last) = [0;0;0;0;0;0;0];
    end
...
function Qd = IK_Panda(qd, qd_dot)
global delta_t
    Qd = qd + qd_dot*delta_t;
end
